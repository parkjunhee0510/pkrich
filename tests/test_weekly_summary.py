...
def _analysis(ticker: str, name: str, signal: str) -> TickerAnalysis:
    return TickerAnalysis(
        ...
        news_references=[NewsItem(title=f"{name} headline", source="Reuters", published_at="2026-04-08", link="https://example.com")],
        financial_highlights=["Revenue: 1.00T"],
        risks_or_watchpoints=["Risk checkpoint"],
                ...
                "tickers": [
                    {
                        ...
                        "news_references": [{"title": "Apple AI headline", "source": "Reuters", "published_at": day, "link": "https://example.com/apple-ai"}],
                        "signal_or_takeaway": "AAPL bullish",
                        "data_snapshot": {"Price": ticker_prices[day]["AAPL"], "Daily Change": "+1.00%", "Sector": "Technology"},
                    },
                    {
                        "ticker": "MSFT",
                        ...
                        "news_references": [{"title": "Microsoft cloud headline", "source": "Yahoo Finance", "published_at": day, "link": "https://example.com/msft-cloud"}],
                        "signal_or_takeaway": "MSFT watch",
                        "data_snapshot": {"Price": ticker_prices[day]["MSFT"], "Daily Change": "-1.00%", "Sector": "Technology"},
                    },
                ],
            }
        )

    (data_dir / "dashboard.json").write_text(json.dumps({"days": days, "signal_stats": {}}, ensure_ascii=False, indent=2), encoding="utf-8")
    (data_dir / "price_history.csv").write_text(
        "\n".join(
            [
                "date,ticker,price,daily_change,market_cap,trailing_pe,eps,52w_high,52w_low",
                *[f"{day},AAPL,{ticker_prices[day]['AAPL']},+1.00%,1.00T,25.0,5.0,120.0,80.0" for day in trading_days],
                *[f"{day},MSFT,{ticker_prices[day]['MSFT']},-1.00%,2.00T,30.0,6.0,240.0,150.0" for day in trading_days],
            ]
        ),
        encoding="utf-8",
    )
...
class WeeklySummaryTests(unittest.TestCase):
    def test_load_weekly_summary_aggregates_weekly_report(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            record_signals(
                [
                    _analysis("AAPL", "Apple Inc.", "AAPL bullish"),
                    _analysis("MSFT", "Microsoft Corporation", "MSFT watch"),
                ],
                date(2026, 4, 1),
            ...
            self.assertTrue(summary.weekly_report)
            self.assertIn("market_environment", summary.weekly_report)
            self.assertIn("top_movers", summary.weekly_report)
            self.assertEqual(summary.weekly_insight, summary.weekly_report.get("summary", ""))
