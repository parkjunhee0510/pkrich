...
* the web dashboard action-change feed, optional today decision strip component, and ticker detail UI may render search evidence metadata as badges or panels; these presentation cues must not mutate or reinterpret official decisions
...
### Strategy Simulator Output

`strategy_simulator.json` is a read-only historical simulation artifact for the Backtest page. It uses `mode: "observational_long_only"` and `basis: "final_action"` to model how finalized `buy` / `watch` / `avoid` recommendations would have behaved as a long-only portfolio. It compares the Korean presets `보수형`, `균형형`, and `공격형`, models next-trading-day open entries, later `avoid` exits, stop-loss, take-profit, fees, slippage, cash limits, position limits, realized P&L, unrealized P&L, open positions, skipped entries, latest BUY entry candidates, and LLM direction alignment diagnostics.

Each preset includes an additive `entry_candidates[]` list for the latest finalized BUY signal per ticker. Candidate rows include rank, ticker, current status, Korean status label, signal date, conviction, possible next entry date/price, stop-loss and take-profit reference prices, target notional, available cash, and LLM direction alignment. These rows are presentation diagnostics for the Backtest page only; they do not create orders, mutate portfolio state, or override finalized decisions.

Entry candidate rows also include additive `news_evidence` diagnostics. This object contains a deterministic score, strength label, normalized news tone, LLM direction/alignment, catalyst/source metadata, reason chips, and a short summary. It is explanatory presentation data only; it must not change candidate ranking, official decisions, or execution behavior.

The root `today_action_queue` is a read-only operating view derived from the `balanced` preset. It groups current entry candidates into `enter`, `watch`, and `skip` queues, adds `hold` alerts for balanced open positions, and includes display-only `action_score` / `alert_score` values, reason chips, stable reason codes, and nested copies of the original candidate or position row. This section is presentation-only: it does not create official recommendations, alter entry-candidate ordering, mutate portfolio state, call external data providers, call the LLM, or trigger trading actions.

The root payload includes `news_shadow`, a read-only signal-quality section for the news-only strategy `strong_news_llm_bull`. It evaluates strong news evidence plus LLM-bull signals against forward 1D, 5D, and 20D returns using next-trading-day open entries. This shadow section is observational and must not alter portfolio simulation, factor weights, model routing, or official recommendation outputs.

The artifact is generated from existing `signal_tracker.csv` and `price_history.csv` rows only. It performs no data fetching, LLM calls, decision recomputation, portfolio mutation, or live trading action. Frontend consumers must display the finalized JSON as-is and must not recompute trades, equity curves, stops, take-profits, P&L, or diagnostics.
...
The local ticker-detail options live panel is outside the output contract. Its delayed option second aggregates are streamed through the local API/WebSocket relay for browser display only; they must not be exported, mirrored, backfilled, or used to change finalized dashboard JSON, ticker histories, decisions, portfolio state, or operational reports.
...
Core dashboard JSON writes, including `dashboard_history.json`, use the shared safe JSON writer with short retry handling so transient OneDrive or antivirus file locks do not abort the output stage. Price history JSON/CSV exports use the same short retry path because `price_history.csv` is frequently touched by the pipeline and local sync tools.
...
`risk_intel_graph.json`, `risk_intel_summary.json`, and `risk_intel_refresh_log.json` are mirrored to `web/public/output/data/` when present. They are read-only risk explanation artifacts for graph/card rendering and must not override official rule-based investment decisions, recompute alerts in the frontend, or trigger provider calls from the output layer. The main dashboard does not render risk intelligence panels; `/risk-intel` renders the Korean card summary, fuller network map, and health-warning context. `/policy` remains the policy report surface; risk intelligence presents how finalized policy/security/social issues propagate into sectors, holdings, and watchlist tickers.

`backtest_summary.json` is mirrored to `web/public/output/data/backtest_summary.json` when present. It is a read-only performance review artifact for the Backtest page and must not affect official decisions. The payload includes root status/strategy/signal counts, optional direction summaries, optional equity curve points, optional ticker rows, and optional signal metadata. Its cumulative return and equity curve treat signals opened in the same 20-trading-day evaluation window as concurrent equal-weight exposure and compound only across non-overlapping windows, so overlapping signals are not reported as sequential full-capital bets.

`strategy_simulator.json` is mirrored to `web/public/output/data/strategy_simulator.json` when present. It is a read-only long-only strategy simulation artifact for the Backtest page and must not affect official decisions, model routing, factor weights, portfolio state, or live trading behavior. The payload includes root status/mode/basis metadata, source row counts, assumptions, preset params and summaries, equity curve points, closed trades, open positions, latest BUY entry candidates, `today_action_queue`, skipped entries, news shadow telemetry, and LLM direction diagnostics.
