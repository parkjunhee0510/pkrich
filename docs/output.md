...
`analysis_performance.json` is a shadow analytics artifact. It includes the shared `schema_version`, `as_of`, `summary`, `signal_performance`, `conviction_calibration`, `regime_performance`, `factor_attribution`, and `action_change_reasons`. It summarizes signal performance, conviction calibration, regime performance, factor attribution, and ticker action-change reasons from finalized decisions and persisted signal rows. It must not recompute, override, or feed back into official decisions, and factor attribution is reported as observed association rather than causal proof.
