...
class ValuationFactor(DecisionFactor):
    ...
    def score(
        ...
        if not vs:
            return FactorScore(value=10, confidence=0.3, reasoning="밸류에이션 데이터가 제한적이라 중립 반영")

        score_val = parse_float(vs.get("score", ""))
        if score_val is None:
            return FactorScore(value=10, confidence=0.3, reasoning="밸류에이션 점수를 읽지 못해 중립 반영")

        scaled = max(0, min(20, score_val * 2))
